    ...
    <div className="flex flex-col min-h-screen">
      ...
      <ul className="flex flex-col divide-y divide-zinc-100 flex-1">
        {optimisticPlayers.map((player) => {
          ...
          let rowBg = "bg-white";
          if (isPaid) rowBg = "bg-green-50";
          else if (isExc) rowBg = "bg-amber-50";

          return (
            <li key={player.id} className={`${rowBg} transition-colors`}>
              <div className="flex min-h-[64px] items-center gap-3 px-3 py-3">
                {/* Jersey number */}
                <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded bg-zinc-100 text-sm font-black text-zinc-600">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  {isExc && player.note && !isExcEditing && (
                    <button
                      type="button"
                      onClick={() =>
                        setExcEdit({ playerId: player.id, draft: player.note ?? "" })
